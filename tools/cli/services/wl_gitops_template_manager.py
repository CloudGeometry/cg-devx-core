...
from common.const.const import WL_GITOPS_REPOSITORY_URL, WL_GITOPS_REPOSITORY_BRANCH
from common.tracing_decorator import trace


class WorkloadGitOpsTemplateManager:
    """CG DevX Workload GitOps templates manager."""

    def __init__(self, org_name: str, repo_name: str, key_path: str, template_url: str = None,
                 template_branch: str = None):
        if template_url is None:
            self._url = WL_GITOPS_REPOSITORY_URL
        else:
            self._url = template_url

        if template_branch is None:
            self._branch = WL_GITOPS_REPOSITORY_BRANCH
        else:
